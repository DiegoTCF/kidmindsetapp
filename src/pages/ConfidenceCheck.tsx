...
const sections: Section[] = [
  {
    title: "Preparation",
    items: [
      { key: "prep_yoga", label: "Yoga/stretch done?" },
      { key: "prep_breathing", label: "Breathing done?" },
      { key: "prep_visual", label: "Visualisation done?" }
    ]
  },
  {
    title: "Plan",
    items: [
      { key: "plan_super_behaviours", label: "Picked Super Behaviours before playing?" },
      { key: "plan_reminder", label: "Reminded myself of game plan?" }
    ]
  },
  {
    title: "Know Who You Are",
    items: [
      { key: "worth_perfect", label: "Reminded \"I don't need to be perfect\"?" },
      { key: "worth_strengths", label: "Trusted my strengths/technique?" },
      { key: "worth_good_anyway", label: "Knew I'm still a good player even on a bad day?" }
    ]
  },
  {
    title: "Beating My Mind (ANTs)",
    items: [
      { key: "ants_notice", label: "Noticed negative thoughts?" },
      { key: "ants_selftalk", label: "Used self-talk to reset?" },
      { key: "ants_no_freeze", label: "Didn't let thoughts freeze me?" }
    ]
  },
  {
    title: "Dealing with Challenges",
    items: [
      { key: "challenges_keep_going", label: "Kept going when it got hard?" },
      { key: "challenges_reset_fast", label: "Reset quickly after a mistake?" },
      { key: "challenges_behaviours", label: "Stayed focused on behaviours?" }
    ]
  }
];
...
export default function ConfidenceCheck() {
  ...
  const stats = useMemo(() => {
    ...
    if (confidencePct >= 70) badge = "Ready";
    else if (confidencePct >= 40) badge = "Almost";

    return { yesCount, confidencePct, nervesPct, sectionScores, badge };
  }, [answers]);
  ...
  const handleSave = () => {
    console.log('Saving data:', { answers, notes, stats });
    // TODO: Save to Supabase
  };

  const handleShare = () => {
    console.log('Sharing with coach:', { answers, notes, stats });
        ...
        <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge 
                variant={stats.confidencePct >= 70 ? "default" : stats.confidencePct >= 40 ? "secondary" : "destructive"}
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {stats.sectionScores.map((section, index) => (
                <Badge 
                  key={index} 
                  variant="outline" 
                ...
                return (
                  <div 
                    ...
                    className={cn(
                      ...
                      isChecked && "border-green-500 bg-green-50/50 dark:bg-green-950/20"
                    )}
                  >
