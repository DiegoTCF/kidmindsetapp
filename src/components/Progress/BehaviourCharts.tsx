...
const BEHAVIOUR_CONFIG = {
  brave_on_ball: {
    ...
    questions: ["How often did you try to take players on or play forward?", "How much intent did you show when doing it — did you really go for it?", "Did you take risks even when you made mistakes or lost the ball?", "How much did you play to win your 1v1s, not just avoid losing the ball?"],
    scales: ["(1 = Not at all, 10 = All the time)", "(1 = Hesitant / Half-hearted, 10 = Full commitment every time)", "(1 = I avoided it after mistakes, 10 = I kept trying and stayed brave)", "(1 = Playing it safe, 10 = Attacking every 1v1 with purpose)"]
  },
  brave_off_ball: {
    icon: "brain",
    title: "Brave off the Ball",
    subtitle: "Getting into the game, showing for the ball, staying involved",
    questions: ["How often did you show for the ball or move into space?", "How much intent did you show when trying to get involved?", "Did you keep moving even when things weren't going well?", "Did you create good angles and options for your teammates?"],
    scales: ["(1 = I hid or waited, 10 = I was always available and active)", "(1 = Passive movements, 10 = Sharp, purposeful movements)", "(1 = I gave up a bit, 10 = I kept trying no matter what)", "(1 = Rarely, 10 = Constantly helped the team with my positioning)"]
  },
  electric: {
    icon: "trophy",
    title: "Electric",
    subtitle: "Energy, speed, quick reactions, intensity",
    questions: ["How much energy did you bring to the game today?", "How quick were your reactions during the game?", "How fast and sharp were your decisions?", "Did you move with speed and urgency when the team needed it?"],
    scales: ["(1 = Very flat, 10 = Full of energy the whole time)", "(1 = Slow to react, 10 = Switched on and alert)", "(1 = I delayed or hesitated, 10 = I made fast, confident choices)", "(1 = I jogged or walked a lot, 10 = I exploded into actions)"]
  },
  aggressive: {
    icon: "target",
    title: "Aggressive",
    subtitle: "Competing, pressing, tackling, 1v1 duels",
    questions: ["How often did you go into 1v1 duels or physical challenges?", "When you pressed or challenged, how committed were you?", "How often did you win your battles or at least make it difficult?", "How much did you enjoy competing and fighting for the ball?"],
    scales: ["(1 = Avoided them, 10 = Went into everything)", "(1 = Soft / hesitant, 10 = 100% effort every time)", "(1 = Lost most or backed out, 10 = Made it a real fight every time)", "(1 = Didn't enjoy it, 10 = Loved the challenge and looked for it)"]
  }
};
  ...
  return <div className="relative" style={{
    ...
    height: size
      ...
      <svg className="transform -rotate-90" width={size} height={size}>
        ...
        <circle cx={size / 2} cy={size / 2} r={radius} stroke="hsl(var(--primary))" strokeWidth={strokeWidth} fill="none" strokeDasharray={strokeDasharray} strokeLinecap="round" className="transition-all duration-500" />
      </svg>
...
const SmallCircularProgress = ({
  ...
  return <div className="relative" style={{
    ...
    height: size
      ...
      <svg className="transform -rotate-90" width={size} height={size}>
        <circle cx={size / 2} cy={size / 2} r={radius} stroke="hsl(var(--muted))" strokeWidth={strokeWidth} fill="none" />
        <circle cx={size / 2} cy={size / 2} r={radius} stroke="hsl(var(--primary))" strokeWidth={strokeWidth} fill="none" strokeDasharray={strokeDasharray} strokeLinecap="round" className="transition-all duration-300" />
      </svg>
      <div className="absolute inset-0 flex items-center justify-center">
...
export default function BehaviourCharts({
  ...
  const loadBehaviourData = async () => {
    try {
      ...
      const {
        data: ratings,
        error
      } = await supabase.from('super_behaviour_ratings').select('*').eq('child_id', targetChildId).order('created_at', {
        ascending: false
      });
  ...
  if (loading) {
    return <div className="space-y-6">
        {Object.keys(BEHAVIOUR_CONFIG).map(behaviourType => <Card key={behaviourType} className="shadow-soft">
  ...
  return <div className="space-y-6">
      ...
      if (!data) {
        return <Card key={behaviourType} className="shadow-soft">
              <CardContent className="p-6">
      ...
      const questionPercentages = [data.question_1_rating / 10 * 100, data.question_2_rating / 10 * 100, data.question_3_rating / 10 * 100, data.question_4_rating / 10 * 100];
      return <Card key={behaviourType} className="shadow-soft">
            <CardContent className="p-6 bg-green-400">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {config.questions.map((question, index) => <div key={index} className="text-center space-y-3">
                    <div className="flex justify-center">
                      <SmallCircularProgress percentage={questionPercentages[index]} size={70} strokeWidth={5} />
                    </div>
